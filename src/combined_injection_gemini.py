...
def load_metadata() -> list[dict]:
    meta_path = os.path.join(AUGMENTED_DIR, "metadata.csv")
    if not os.path.exists(meta_path):
        raise FileNotFoundError(
            f"Could not find '{meta_path}'. Run generate_noise_augmented_dataset.py first."
        )
...
def main():
    ...
    # Filter to selected pairs only
    records = [r for r in all_records if int(r["id"]) in SELECTED_PAIRS]

    results_csv = os.path.join(RESULTS_DIR, "results_combined_injection_v3.csv")
    processed   = set()
    results     = []
    ...
    for record in records:
        ...
        audio_path = os.path.join(AUGMENTED_DIR, audio_file)
            ...
            with open(results_csv, "w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(
                    f, fieldnames=["id", "run", "prompt", "response",
                                   "audio_file", "snr_db", "gemini_response"]
                )
                writer.writeheader()
